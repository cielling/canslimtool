...
def analyzeTickers(df, procNum = 0):
    ...
    with sqlite3.connect('edgar_idx.db') as conn:
        ...
        with open("analysislog"+str(procNum)+".txt", mode) as logfile:
            with open("analysiserrors"+str(procNum)+".txt", "w") as errorlog:
                ...
                for symbol in df.Symbol.sort_values():
                    ...
                    df.to_excel("screener_results_analysis"+str(procNum)+".xls")
                    print("Processing {:s}\n".format(symbol))
